...
public class AdventureGame extends GUI {
  ...
  protected String askSave() {
    ...
    int ans = JOptionPane.showConfirmDialog(container,
        "Would you like to save your game before you leave?", null, JOptionPane.YES_NO_OPTION);
    ...
    for (PickUpAbleStrategy item : player.getInventory()) {
      InventoryDisplay inventoryImageComponent = new InventoryDisplay(item) {
        ...
        public void paintComponent(Graphics g) {
          ...
          g.drawImage(scaledImage, 2, 2, InventoryDisplay.IMAGE_WIDTH - 2,
              InventoryDisplay.IMAGE_HEIGHT - 2, null);

        }
      };
      inventoryImageComponent.setPreferredSize(
          new Dimension(InventoryDisplay.IMAGE_WIDTH, InventoryDisplay.IMAGE_HEIGHT));
  ...
  protected void doRelease(MouseEvent e) {
    ...
    if (clickedLocation != null) {
      System.out
          .println("row[" + clickedLocation.getRow() + "], col[" + clickedLocation.getCol() + "]");
    }
  }
